...
# Run one R3LIVE official-dataset experiment.
#
# Clean:
#   MODE=clean RUN_NAME=clean_01 bash run_r3live_official_once.sh
#
# Attack, after clean trajectory exists:
#   MODE=static RUN_NAME=static_01 SPOOFER_X=... SPOOFER_Y=... \
#     REF_CSV=.../clean_01_traj.csv bash run_r3live_official_once.sh

MODE="${MODE:-clean}"              # clean | static | removal
METHOD_LABEL="${METHOD_LABEL:-r3live}"
RUN_NAME="${RUN_NAME:-r3live_${MODE}_01}"
...
SEQUENCE="${SEQUENCE:-hku_campus_seq_00}"
BAG="${BAG:-$DATA_ROOT/raw_rosbags/R3LIVE-Dataset/${SEQUENCE}.bag}"
...
cleanup() {
    ...
    safe_kill "$TARGET_PID" "R3LIVE roslaunch"
...
echo "[INFO] Launching R3LIVE ..."
roslaunch slamspoof_icra r3live_official.launch \
    rviz:="$RVIZ" \
    map_output_dir:="$RUN_DIR/map" \
    record_offline_map:="$RECORD_OFFLINE_MAP" \
    > "$RUN_DIR/02_r3live.log" 2>&1 &
TARGET_PID=$!
sleep "$TARGET_START_WAIT"
if ! kill -0 "$TARGET_PID" 2>/dev/null; then
    echo "[ERROR] R3LIVE exited early. Check $RUN_DIR/02_r3live.log" >&2
    exit 1
fi
...
if [[ "$MODE" != "clean" && -n "$REF_CSV" ]]; then
    python3 "$SLAMSPOOF_DIR/scripts/evaluate_attack.py" \
        ...
        --att "$TRAJ_CSV" \
        --out-dir "$EVAL_DIR" \
        --title "R3LIVE official: $MODE $RUN_NAME" \
        --spoofer-x "$SPOOFER_X" \
        --spoofer-y "$SPOOFER_Y" \
...
SUMMARY_METHOD="$METHOD_LABEL"
SUMMARY_SPOOFER_X="${SPOOFER_X:-NA}"
SUMMARY_SPOOFER_Y="${SPOOFER_Y:-NA}"
if [[ "$MODE" == "clean" ]]; then
    SUMMARY_METHOD="r3live"
    SUMMARY_SPOOFER_X="NA"
    SUMMARY_SPOOFER_Y="NA"
fi
echo "${RUN_NAME},${SUMMARY_METHOD},${MODE},${DISTANCE_THRESHOLD},${SPOOFING_RANGE},${SUMMARY_SPOOFER_X},${SUMMARY_SPOOFER_Y},${RUN_BAG},${TRAJ_BAG},${TRAJ_CSV},${EVAL_DIR},${APE},${RPE1},${RPE10},${RPEMAX},${STATUS}" >> "$SUMMARY_CSV"

if [[ "$MODE" != "clean" && "$KEEP_ATTACK_BAG" -eq 0 ]]; then
    rm -f "$ATTACK_BAG" "$ATTACK_BAG.active"
fi

echo "[OK] R3LIVE $MODE run complete:"
