...
#   3) launch LVI-SAM
#   4) record odometry
#   5) play attacked bag with --clock -r 1.0
...
OUT_ROOT="${OUT_ROOT:-$LVI_DATASET_DIR/repeat_static/smvs_${PARAM_TAG}_x${N_RUNS}}"
...
for RUN_INDEX in $(seq "$START_RUN" "$N_RUNS"); do
    ...
    echo "============================================================"
    echo "[RUN $RUN_ID/$N_RUNS] handheld smvs static D=${DISTANCE_THRESHOLD} range=${SPOOFING_RANGE} tag=${PARAM_TAG}"
    echo "============================================================"

    RUN_DIR="$RUNS_DIR/run_${RUN_ID}"
    mkdir -p "$RUN_DIR"

    ATTACK_BAG="$RUN_DIR/handheld_attack_static_smvs_${PARAM_TAG}_x${N_RUNS}_run_${RUN_ID}.bag"
    RUN_CONFIG="$RUN_DIR/config_smvs_static_${PARAM_TAG}_${RUN_ID}.json"
    TRAJ_BAG="$RUN_DIR/handheld_attack_static_smvs_${PARAM_TAG}_x${N_RUNS}_run_${RUN_ID}_traj.bag"
    TRAJ_CSV="$RUN_DIR/handheld_attack_static_smvs_${PARAM_TAG}_x${N_RUNS}_run_${RUN_ID}_traj.csv"
    ...
    python3 "$SLAMSPOOF_DIR/scripts/evaluate_attack.py" \
        ...
        --att "$TRAJ_CSV" \
        --out-dir "$EVAL_DIR" \
        --title "handheld smvs static D${DISTANCE_THRESHOLD} R${SPOOFING_RANGE} run ${RUN_ID}" \
        --spoofer-x "$SPOOFER_X" \
        --spoofer-y "$SPOOFER_Y" \
