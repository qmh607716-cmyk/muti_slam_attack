...
# ============================================================
# Batch experiment:
# handheld / SMVS / removal / D=15m / spoofing_range=80deg
#
# Each run:
...
OUT_ROOT="$LVI_DATASET_DIR/repeat_removal/smvs_1580_x15"
...
for RUN_INDEX in $(seq "$START_RUN" "$N_RUNS"); do
    ...
    echo "============================================================"
    echo "[RUN $RUN_ID/$N_RUNS] handheld smvs removal D=15 range=80 x15"
    echo "============================================================"
    ...
    ATTACK_BAG="$RUN_DIR/handheld_attack_removal_smvs_1580_x15_run_${RUN_ID}.bag"
    RUN_CONFIG="$RUN_DIR/config_smvs_removal_${RUN_ID}.json"
    TRAJ_BAG="$RUN_DIR/handheld_attack_removal_smvs_1580_x15_run_${RUN_ID}_traj.bag"
    TRAJ_CSV="$RUN_DIR/handheld_attack_removal_smvs_1580_x15_run_${RUN_ID}_traj.csv"
    ...
    python3 "$SLAMSPOOF_DIR/scripts/evaluate_attack.py" \
        ...
        --att "$TRAJ_CSV" \
        --out-dir "$EVAL_DIR" \
        --title "handheld smvs removal D15 R80 run ${RUN_ID}" \
        --spoofer-x "$SPOOFER_X" \
        --spoofer-y "$SPOOFER_Y" \
