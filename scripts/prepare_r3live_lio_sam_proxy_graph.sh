...
# Build an attacker-side LIO-SAM proxy graph for the R3LIVE official bag.
# The generated dump directory can be consumed by
# prepare_r3live_method_positions.sh via GRAPH_SOURCE=lio_sam.
...
if [[ ! -f "$BAG" ]]; then
    echo "[ERROR] R3LIVE bag not found: $BAG" >&2
    exit 2
fi
...
if [[ "$CONVERT_BAG" -eq 1 || ! -f "$LIO_BAG" ]]; then
    echo "[STEP] Convert R3LIVE Livox bag to LIO-SAM proxy bag"
...
roslaunch slamspoof_icra r3live_lio_sam_proxy.launch \
