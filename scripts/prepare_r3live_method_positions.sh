...
# Build R3LIVE official-dataset SMVS/Bi-SMVS placement inputs and select
...
R3LIVE_CONFIG="${R3LIVE_CONFIG:-$HOME/catkin_ws/src/R3LIVE/config/r3live_config.yaml}"
EXTRINSIC_DIRECTION="${EXTRINSIC_DIRECTION:-camera_to_lidar}"
IMG_TIME_OFFSET="${IMG_TIME_OFFSET:-0.0}"
...
if [[ ! -f "$CLEAN_CSV" ]]; then
    ...
    echo "        Run MODE=clean RUN_NAME=clean_01 run_r3live_official_once.sh first." >&2
    exit 2
fi
...
if [[ "$RECOMPUTE_VULNERABILITY" -eq 1 || ! -f "$SMVS_CSV" || ! -f "$BISMVS_CSV" ]]; then
    echo "[STEP] Compute R3LIVE SMVS/Bi-SMVS CSVs"
    compute_args=(
        --bag "$BAG"
        --traj "$CLEAN_CSV"
        --out-root "$VUL_ROOT"
        ...
        --r3live-config "$R3LIVE_CONFIG"
        --extrinsic-direction "$EXTRINSIC_DIRECTION"
        --img-time-offset "$IMG_TIME_OFFSET"
    )
    if [[ -n "$MAX_FRAMES" ]]; then
        compute_args+=(--max-frames "$MAX_FRAMES")
    fi
    python3 "$SLAMSPOOF_DIR/scripts/compute_r3live_vulnerability.py" "${compute_args[@]}"
else
    echo "[SKIP] Reusing existing vulnerability CSVs under $VUL_ROOT"
fi

echo "[STEP] Select R3LIVE SMVS position with paper III-C selector"
...
echo "[STEP] Select R3LIVE Bi-SMVS position with proxy-assisted selector"
...
echo "[OK] R3LIVE method positions are ready:"
echo "     $METHOD_POS_CSV"
