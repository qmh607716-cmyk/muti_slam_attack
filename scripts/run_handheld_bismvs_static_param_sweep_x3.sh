...
# LVI-SAM handheld parameter sensitivity sweep for Bi-SMVS static attack.
# The spoofer position is fixed to the current Bi-SMVS selected location.
# Only the angular spoofing range changes, so the sweep isolates attack strength.
...
run_one_range() {
    ...
    echo
    echo "============================================================"
    echo "[SWEEP] Bi-SMVS static D=${DISTANCE_THRESHOLD} range=${range} x${N_RUNS}"
