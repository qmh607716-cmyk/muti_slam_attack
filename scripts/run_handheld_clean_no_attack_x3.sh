...
# handheld / no attack / clean replay / 3 runs
#
# Each run:
...
# Output root for 3 clean replay runs.
OUT_ROOT="$LVI_DATASET_DIR/repeat_clean/no_attack_x3"
...
append_summary_row() {
    ...
    echo "${run_id},${PLATFORM},${METHOD},${MODE},${DISTANCE_THRESHOLD},${SPOOFING_RANGE},${SPOOFER_X},${SPOOFER_Y},${CLEAN_BAG},${traj_bag},${traj_csv},${eval_dir},${ape_rmse},${rpe1_rmse},${rpe10_rmse},${status}" >> "$SUMMARY_CSV"
}
...
run,platform,method,mode,distance_threshold,spoofing_range,spoofer_x,spoofer_y,input_bag,traj_bag,traj_csv,eval_dir,ape_rmse,rpe_1m_rmse,rpe_10m_rmse,status
EOF2
else
...
for RUN_INDEX in $(seq "$START_RUN" "$N_RUNS"); do
    ...
    echo "============================================================"
    echo "[RUN $RUN_ID/$N_RUNS] handheld clean no-attack replay"
    echo "============================================================"
...
if apes:
    print(f"  APE min    : {min(apes):.4f} m")
    print(f"  APE median : {statistics.median(apes):.4f} m")
    print(f"  APE mean   : {statistics.mean(apes):.4f} m")
    print(f"  APE max    : {max(apes):.4f} m")
    if len(apes) > 1:
        print(f"  APE std    : {statistics.stdev(apes):.4f} m")
    print()
    print("  Per-row APE:")
    for run_id, ape, status in rows:
        if ape is None:
            ...
            print(f"    {run_id}: {ape:10.4f} m  [{status}]")
PY
