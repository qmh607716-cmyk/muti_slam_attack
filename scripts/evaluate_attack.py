...
def run_evo(orig_tum: str, att_tum: str, out_dir: str, title: str) -> dict:
    """Run evo_ape (ATE) and evo_rpe, return parsed results."""
    import zipfile
    results = {"ape": {}, "rpe_1m": {}, "rpe_10m": {}}
...
def main():
    ...
    ape_stats = load_evo_zip(os.path.join(out_dir, "evo_ape.txt"))
    rpe1_stats = load_evo_zip(os.path.join(out_dir, "evo_rpe.txt"))
    rpe10_stats = load_evo_zip(os.path.join(out_dir, "evo_rpe_10m.txt"))
    ...
    print("\n-- evo ATE (Absolute Trajectory Error, Umeyama aligned) --")
    if ape_stats:
        for k in ["rmse", "mean", "median", "std", "min", "max"]:
            if k in ape_stats:
                print(f"  {k}: {ape_stats[k]:.4f} m")
    else:
        print("  (evo_ape not available)")

    print("\n-- evo RPE (Relative Pose Error, delta=1m) --")
    if rpe1_stats:
        for k in ["rmse", "mean", "median", "std", "min", "max"]:
            if k in rpe1_stats:
                print(f"  {k}: {rpe1_stats[k]:.4f} m")
    else:
        print("  (evo_rpe 1m not available)")

    print("\n-- evo RPE (Relative Pose Error, delta=10m) --")
    if rpe10_stats:
        for k in ["rmse", "mean", "median", "std", "min", "max"]:
            if k in rpe10_stats:
                print(f"  {k}: {rpe10_stats[k]:.4f} m")
    else:
        print("  (evo_rpe 10m not available)")
    ...
    print(f"  Max deviation at:  t={metrics['duration_s'] * raw['max_3d_idx'] / raw['max_3d_idx']:.1f}s (idx={raw['max_3d_idx']})")
    print("  Per-axis RMSE:")
    ...
    complete = {
        "title": args.title,
        "evo": {"ate": ape_stats, "rpe_1m": rpe1_stats, "rpe_10m": rpe10_stats},
        "raw_error": raw,
