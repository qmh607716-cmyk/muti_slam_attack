...
OUT_ROOT="${OUT_ROOT:-$LVI_DATASET_DIR/repeat_removal/bismvs_${PARAM_TAG}_x${N_RUNS}}"
...
for RUN_INDEX in $(seq "$START_RUN" "$N_RUNS"); do
    ...
    echo "============================================================"
    echo "[RUN $RUN_ID/$N_RUNS] handheld bismvs removal D=${DISTANCE_THRESHOLD} range=${SPOOFING_RANGE} tag=${PARAM_TAG}"
    echo "============================================================"

    RUN_DIR="$RUNS_DIR/run_${RUN_ID}"
    mkdir -p "$RUN_DIR"

    ATTACK_BAG="$RUN_DIR/handheld_attack_removal_bismvs_${PARAM_TAG}_x${N_RUNS}_run_${RUN_ID}.bag"
    RUN_CONFIG="$RUN_DIR/config_bismvs_removal_${PARAM_TAG}_${RUN_ID}.json"
    TRAJ_BAG="$RUN_DIR/handheld_attack_removal_bismvs_${PARAM_TAG}_x${N_RUNS}_run_${RUN_ID}_traj.bag"
    TRAJ_CSV="$RUN_DIR/handheld_attack_removal_bismvs_${PARAM_TAG}_x${N_RUNS}_run_${RUN_ID}_traj.csv"
    ...
    python3 "$SLAMSPOOF_DIR/scripts/evaluate_attack.py" \
        ...
        --att "$TRAJ_CSV" \
        --out-dir "$EVAL_DIR" \
        --title "handheld bismvs removal D${DISTANCE_THRESHOLD} R${SPOOFING_RANGE} run ${RUN_ID}" \
        --spoofer-x "$SPOOFER_X" \
        --spoofer-y "$SPOOFER_Y" \
