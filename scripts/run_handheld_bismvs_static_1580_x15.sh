...
# ============================================================
# Batch experiment:
# handheld / Bi-SMVS / static
# Defaults remain D=15m / spoofing_range=80deg / x15.
#
# Each run:
...
# Attack metadata.
METHOD="${METHOD:-bismvs}"
METHOD_SLUG="${METHOD_SLUG:-$METHOD}"
PLATFORM="handheld"
MODE="static"

# Output root for this repeated experiment.
OUT_ROOT="${OUT_ROOT:-$LVI_DATASET_DIR/repeat_static/${METHOD_SLUG}_${PARAM_TAG}_x${N_RUNS}}"
...
# Give rosbag record enough time to subscribe before replay.
RECORD_START_WAIT=2
...
# Graceful stop wait after SIGINT.
STOP_WAIT=5

# Minimum acceptable CSV row count for a run to be considered successful.
# handheld bag duration is around 1600s; odometry roughly several Hz.
# This catches obviously truncated runs.
MIN_CSV_ROWS=6000

# Storage control.
# 0 = delete recorded trajectory bag after CSV extraction/evaluation.
# 1 = keep every trajectory bag. This costs much more disk space.
KEEP_TRAJ_BAG=0

# Whether to regenerate attacked bag at every run.
# 1 = full repetition, matches manual workflow.
# 0 = generate only once and reuse attacked bag.
REGENERATE_ATTACK_BAG_EACH_RUN=1

# Whether to delete the attacked input bag after each run.
# Since REGENERATE_ATTACK_BAG_EACH_RUN=1, this saves disk safely.
# If you set REGENERATE_ATTACK_BAG_EACH_RUN=0, set this to 0.
...
cleanup_lvi_processes() {
    ...
    # Kill LVI-SAM launch and nodes if any remain.
    pkill -f "roslaunch.*lvi_sam.*run.launch" >/dev/null 2>&1 || true
    pkill -f "lvi_sam" >/dev/null 2>&1 || true

    # Kill leftover rosbag record on the odom topic if any.
...
cleanup_large_outputs_after_run() {
    # Remove recorded trajectory bag to save disk.
    if [[ "${KEEP_TRAJ_BAG}" -eq 0 ]]; then
        rm -f "$TRAJ_BAG" "$TRAJ_BAG.active" 2>/dev/null || true
    fi

    # Remove attacked bag after current run if it will be regenerated next time.
...
append_summary_row() {
    ...
    echo "${run_id},${PLATFORM},${METHOD},${MODE},${DISTANCE_THRESHOLD},${SPOOFING_RANGE},${SPOOFER_X},${SPOOFER_Y},${ATTACK_BAG},${traj_bag},${traj_csv},${eval_dir},${ape_rmse},${rpe1_rmse},${rpe10_rmse},${rpe1_max},${rpe10_max},${rpe_max},${status}" >> "$SUMMARY_CSV"
}
...
# -----------------------------
# Start ROS master inside script
# -----------------------------
...
if [[ "$RESET_SUMMARY" -eq 1 ]]; then
    ...
    cat > "$SUMMARY_CSV" <<EOF
run,platform,method,mode,distance_threshold,spoofing_range,spoofer_x,spoofer_y,attack_bag,traj_bag,traj_csv,eval_dir,ape_rmse,rpe_1m_rmse,rpe_10m_rmse,rpe_1m_max,rpe_10m_max,rpe_max,status
EOF
else
...
for RUN_INDEX in $(seq "$START_RUN" "$N_RUNS"); do
    ...
    echo "============================================================"
    echo "[RUN $RUN_ID/$N_RUNS] handheld ${METHOD_SLUG} static D=${DISTANCE_THRESHOLD} range=${SPOOFING_RANGE} tag=${PARAM_TAG}"
    echo "============================================================"

    RUN_DIR="$RUNS_DIR/run_${RUN_ID}"
    mkdir -p "$RUN_DIR"

    ATTACK_BAG="$RUN_DIR/handheld_attack_static_${METHOD_SLUG}_${PARAM_TAG}_x${N_RUNS}_run_${RUN_ID}.bag"
    RUN_CONFIG="$RUN_DIR/config_${METHOD_SLUG}_static_${PARAM_TAG}_${RUN_ID}.json"
    TRAJ_BAG="$RUN_DIR/handheld_attack_static_${METHOD_SLUG}_${PARAM_TAG}_x${N_RUNS}_run_${RUN_ID}_traj.bag"
    TRAJ_CSV="$RUN_DIR/handheld_attack_static_${METHOD_SLUG}_${PARAM_TAG}_x${N_RUNS}_run_${RUN_ID}_traj.csv"
    ...
    # Remove old outputs for this run.
    rm -f "$TRAJ_BAG" "$TRAJ_BAG.active" "$TRAJ_CSV"
    rm -rf "$EVAL_DIR"
    mkdir -p "$EVAL_DIR"

    # Clean possible leftovers from previous interrupted run.
    cleanup_lvi_processes
    rosparam set /use_sim_time true
    ...
    # Important: stop LVI-SAM first, keep recorder alive to capture final odometry.
    ...
    python3 "$SLAMSPOOF_DIR/scripts/evaluate_attack.py" \
        ...
        --att "$TRAJ_CSV" \
        --out-dir "$EVAL_DIR" \
        --title "handheld ${METHOD_SLUG} static D${DISTANCE_THRESHOLD} R${SPOOFING_RANGE} run ${RUN_ID}" \
        --spoofer-x "$SPOOFER_X" \
        --spoofer-y "$SPOOFER_Y" \
