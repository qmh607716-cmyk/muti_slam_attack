...
# ============================================================
# Batch experiment:
# handheld / SMVS / static / D=30m / spoofing_range=80deg
#
# Each run:
...
OUT_ROOT="$LVI_DATASET_DIR/repeat_static/smvs_3080"
...
for RUN_INDEX in $(seq "$START_RUN" "$N_RUNS"); do
    ...
    echo "============================================================"
    echo "[RUN $RUN_ID/$N_RUNS] handheld smvs static D=30 range=80"
    echo "============================================================"
    ...
    python3 "$SLAMSPOOF_DIR/scripts/evaluate_attack.py" \
        ...
        --att "$TRAJ_CSV" \
        --out-dir "$EVAL_DIR" \
        --title "handheld smvs static D30 R80 run ${RUN_ID}" \
        --spoofer-x "$SPOOFER_X" \
        --spoofer-y "$SPOOFER_Y" \
