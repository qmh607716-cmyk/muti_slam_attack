...
# D=15m / spoofing_range=80deg
#
# Usage:
#   MODE=static  bash run_handheld_random_spoof_1580_x15.sh
#   MODE=removal bash run_handheld_random_spoof_1580_x15.sh
#
...
OUT_ROOT="$LVI_DATASET_DIR/repeat_${MODE}/random_1580_x15"
...
append_summary_row() {
    ...
    echo "${run_id},${PLATFORM},${METHOD},${MODE},${DISTANCE_THRESHOLD},${SPOOFING_RANGE},${SPOOFER_X},${SPOOFER_Y},${random_id},${anchor_idx},${min_traj_dist},${trigger_frames},${trigger_ratio},${trigger_start_s},${trigger_end_s},${ATTACK_BAG},${traj_bag},${traj_csv},${eval_dir},${ape_rmse},${rpe1_rmse},${rpe10_rmse},${rpe1_max},${rpe10_max},${rpe_max},${status}" >> "$SUMMARY_CSV"
}
...
if [[ "$RESET_SUMMARY" -eq 1 ]]; then
    ...
    cat > "$SUMMARY_CSV" <<EOF
run,platform,method,mode,distance_threshold,spoofing_range,spoofer_x,spoofer_y,random_id,anchor_idx,min_traj_dist,trigger_frames,trigger_ratio,trigger_start_s,trigger_end_s,attack_bag,traj_bag,traj_csv,eval_dir,ape_rmse,rpe_1m_rmse,rpe_10m_rmse,rpe_1m_max,rpe_10m_max,rpe_max,status
EOF
else
...
for RUN_INDEX in $(seq "$START_RUN" "$N_POSITIONS"); do
    ...
    echo "============================================================"
    echo "[RUN $RUN_ID/$N_POSITIONS] handheld random $MODE D=15 range=80 x15"
    ...
    ATTACK_BAG="$RUN_DIR/handheld_attack_${MODE}_random_1580_x15_run_${RUN_ID}.bag"
    RUN_CONFIG="$RUN_DIR/config_random_${MODE}_${RUN_ID}.json"
    TRAJ_BAG="$RUN_DIR/handheld_attack_${MODE}_random_1580_x15_run_${RUN_ID}_traj.bag"
    TRAJ_CSV="$RUN_DIR/handheld_attack_${MODE}_random_1580_x15_run_${RUN_ID}_traj.csv"
    ...
    python3 "$SLAMSPOOF_DIR/scripts/evaluate_attack.py" \
        ...
        --att "$TRAJ_CSV" \
        --out-dir "$EVAL_DIR" \
        --title "handheld random ${MODE} D15 R80 run ${RUN_ID}" \
        --spoofer-x "$SPOOFER_X" \
        --spoofer-y "$SPOOFER_Y" \
        --distance-threshold "$DISTANCE_THRESHOLD" \
        > "$EVAL_LOG" 2>&1
