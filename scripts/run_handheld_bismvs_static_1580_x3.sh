...
# ============================================================
# Batch experiment:
# handheld / Bi-SMVS / static / D=15m / spoofing_range=80deg
#
# Each run:
...
OUT_ROOT="${OUT_ROOT:-$LVI_DATASET_DIR/repeat_static/bismvs_1580}"
...
ATTACK_ANGLE_MODE="${ATTACK_ANGLE_MODE:-lidar_local}"
...
write_run_config() {
    ...
    python3 - "$CONFIG_FILE" "$run_config" "$attack_bag" "$MODE" \
        ...
        "$ATTACK_ANGLE_MODE" <<'PY'
import json
import os
import sys

base_path, out_path, attack_bag, mode, sx, sy, dist_th, spoof_range, angle_mode = sys.argv[1:]
...
cfg["main"]["attack_angle_mode"] = angle_mode
...
echo "[INFO] Mode: $MODE"
echo "[INFO] Attack angle mode: $ATTACK_ANGLE_MODE"
...
for RUN_INDEX in $(seq "$START_RUN" "$N_RUNS"); do
    ...
    echo "============================================================"
    echo "[RUN $RUN_ID/$N_RUNS] handheld bismvs static D=15 range=80"
    echo "============================================================"

    RUN_DIR="$RUNS_DIR/run_${RUN_ID}"
    mkdir -p "$RUN_DIR"

    ATTACK_BAG="$RUN_DIR/handheld_attack_static_bismvs_1580_run_${RUN_ID}.bag"
    RUN_CONFIG="$RUN_DIR/config_bismvs_static_${RUN_ID}.json"
    TRAJ_BAG="$RUN_DIR/handheld_attack_static_bismvs_1580_run_${RUN_ID}_traj.bag"
    TRAJ_CSV="$RUN_DIR/handheld_attack_static_bismvs_1580_run_${RUN_ID}_traj.csv"
    ...
    python3 "$SLAMSPOOF_DIR/scripts/evaluate_attack.py" \
        ...
        --att "$TRAJ_CSV" \
        --out-dir "$EVAL_DIR" \
        --title "handheld bismvs static D15 R80 run ${RUN_ID}" \
        --spoofer-x "$SPOOFER_X" \
        --spoofer-y "$SPOOFER_Y" \
