...
# Run FAST-LIVO2 transfer experiments at D=15m, R=80deg for both placement
...
if [[ ! -f "$POSITION_CSV" ]]; then
    ...
    echo "        Run prepare_fast_livo2_method_positions.sh first." >&2
    exit 2
fi
...
run_one() {
    ...
    if [[ "$SKIP_COMPLETED" -eq 1 && -f "$metrics_path" ]]; then
        ...
        echo "[SKIP] FAST-LIVO2 $run_name already has metrics: $metrics_path"
        return
    fi

    echo
    echo "============================================================"
    echo "[FAST-LIVO2] method=$method mode=$mode run=$run_id/$N_RUNS"
    ...
    SPOOFER_X="$sx" \
    SPOOFER_Y="$sy" \
        bash "$SLAMSPOOF_DIR/scripts/run_fast_livo2_official_once.sh"
}
...
echo
echo "[OK] FAST-LIVO2 method suite complete."
